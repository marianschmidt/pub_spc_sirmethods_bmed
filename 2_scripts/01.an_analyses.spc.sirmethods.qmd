...
output_dir_tables_name <- "3_output/all_figs_tabs"
output_dir_tables      <- here::here(output_dir_tables_name)
...
supp_tab_def_gt <- supp_tab_def %>%
  ...
  #make header
  gt::tab_header(
    title = "Table S1: Comparison of IARC/IACR and SEER multiple primary rules",
    subtitle = "") %>%
  #rename columns
    gt::cols_label(
      "domain" = "",
...
### S7. Figure: Frequency of t_sublung and t_histgroupiarc in SEER and ZfKD
...
supp_fig_hist <- res_hist_freq %>%
  ...
  ggblanket::gg_col(
    ...
    x_title = "",
    y_title = "",
    title = "Fig. S7: Histological groups of LC and SPLC",
...
### S9. Figure: SIR1 vs. SIR2 by Follow-up time
...
supp_fig_byfutime_title <- rlang::englue('Fig. S9: Relative risk for SPLC in lung cancer survivors stratified by follow-up time (n={format(nrow(d1_lung_wide), big.mark = ",")}).')
supp_fig_byfutime_subtitle <- rlang::englue("SIRs stratified by sex on the log-transformed y axis (for females in yellow and for males in blue) and stratified by follow-up time on the x axis. <br>
                                            Top row shows values for unadjusted estimation of risk for SPLC after LC using general reference rates (SIR1~raw~). <br>
                                            Bottom row shows SIR using subtype-specific reference rates excluding same-histology group (SIR2~sub~).")
...
supp_fig_byfutime <- wrap_plots(
  ...
  plot_annotation(
    title = supp_fig_byfutime_title,
        #title = element_text(paste0("Fig. S9: Incidence of SPLC  (n=", format(nrow(d1_lung_wide), big.mark = ","), ")")),
...
### S10. Table: Results from senstivity analysis A
...
```{r}
sensa_tab3_title <- md("S10. Table: Sensitivity analysis A – Risk for SPLC using unadjusted and histology-specific SIR method<br>[restricted to six German PBCR with low DCO rate]")
...
### S11. Table: Results from senstivity analysis B
...
```{r}
sensb_tab3_title <- md("S11. Table: Sensitivity analysis B – Risk for SPLC using unadjusted and histology-specific SIR method <br> [SEER restricted to White population]")
